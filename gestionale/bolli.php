...
title("$sitename: Parametri <i>bolli</i>");
?>
	<script>
...
OpenTable();
echo '<p>';
echo '<input type=button value="Nuovo Record" onclick="location.href=' . chr(39) . 'gestionale.php?name=parametri&subname=bolli&act=new&ord=' . $ord . '&pag='. $pag . chr(39) . '" style="font-family: Verdana; font-size: 10px">';
echo '<input type=button value="Ricerca" onclick="location.href=' . chr(39) . 'gestionale.php?name=parametri&subname=bolli&act=search&ord=' . $ord . '&pag='. $pag . chr(39) . '" style="font-family: Verdana; font-size: 10px">';
echo '<input type=button value="Mostra tutti i record" onclick="location.href=' . chr(39) . 'gestionale.php?name=parametri&subname=bolli' . chr(39) . '" style="font-family: Verdana; font-size: 10px">';
echo '</p>';
echo '<p>';
	if ($act=='sav'){
	...
	if ($act=='savnew'){
		$sql = "INSERT INTO " . $tablename . " (field1,field2) VALUES ('" . $_GET[field1] . "','" . $_GET[field2] . "')";
		$result = $db->sql_query($sql);
		$id = '';
		$act = '';
	}
	...
	$condizioni = " field1 LIKE '$field1' AND field2 LIKE '$field2' ";
	...
	echo '<tr>';
	echo '<th width=10%><font face=verdana size=2><a href=gestionale.php?name=parametri&subname=bolli&ord=id>ID</a></th>';
	echo '<th width=35%><font face=verdana size=2><a href=gestionale.php?name=parametri&subname=bolli&ord=field1>Paese</a></th>';
	echo '<th width=35%><font face=verdana size=2><a href=gestionale.php?name=parametri&subname=bolli&ord=field2>Tasso del bollo</a></th>';
	...
	if ($act == 'search') {
		echo '<form action=gestionale.php method=get><input type=hidden name=ord value=' . $ord . '><input type=hidden name=name value=parametri><input type=hidden name=subname value=bolli>';
	...
	if ($nr != 0){
		...
		if ($id == $row['id'] && $act == 'mod'){
			...
			echo "<td valign=middle align=center><font face=verdana size=2><a href=gestionale.php?name=parametri&subname=bolli&id=" . $row[id] . ">" . $row[id] . "</a></td>";
			...
			echo "<td align=center valign=middle><font face=verdana size=2><a href=gestionale.php?name=parametri&subname=bolli&ord=" . $ord . "&pag=" . $pag . "&act=mod&id=" . $row[id] . "><img border=0 src=immagini/modify.png></a></td>";
			echo "<td align=center valign=middle><font face=verdana size=2><a href=gestionale.php?name=parametri&subname=bolli&ord=" . $ord . "&pag=" . $pag . "&act=del&id=" . $row[id] . " " . $confirm . "><img border=0 src=immagini/remove.png></a></td>";
		}
		echo '</tr>';
		}
	}
