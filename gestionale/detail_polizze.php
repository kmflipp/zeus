...
if ($act == 'savnew'){
	$sql = "INSERT INTO nuke_polizze_detail (id,detail,value) VALUES (" . $id . ",'" . $detail . "','" . $value . "')";
	$db->sql_query($sql);
	$act = 'detail';
}

if ($act == 'del'){
	$sql = "DELETE FROM nuke_polizze_detail WHERE id = " . $id . " AND detail = '" . $detail . "' AND value = '" . $value . "'";
	$db->sql_query($sql);
	$act = 'detail';
}
...
OpenTable();
echo '<p>';
echo '<input type=button value="Nuovo Record" onclick="location.href=' . chr(39) . 'gestionale.php?name=detail_polizze&act=new&id=' . $id . chr(39) . '" style="font-family: Verdana; font-size: 10px">';
echo '<input type=button value="Mostra tutti i record" onclick="location.href=' . chr(39) . 'gestionale.php?name=detail_polizze&id=' . $id . chr(39) . '" style="font-family: Verdana; font-size: 10px">';
echo '</p>';
CloseTable();

OpenTable();
echo '<table width=100% border=1 cellpadding=0 cellspacing=0>';
$sql = "SELECT * FROM nuke_polizze_detail WHERE id = " . $id;
$rs = $db->sql_query($sql);
echo '<tr>';
...
if ($act == 'new') {
	echo '<form action=gestionale.php method=get><input type=hidden name=name value=detail_polizze><input type=hidden name=act value=savnew><input type=hidden name=id value='.$id.'>';
	echo '<tr>';
...
while ($row = $db->sql_fetchrow($rs))
{
	echo '<tr>';
		...
		echo "<td align=center valign=middle><font face=verdana size=2><a href=gestionale.php?name=detail_polizze&act=del&id=" . $row[id] . "&detail=" . $row[detail] . "&value=" . $row[value] . " " . $confirm . "><img border=0 src=immagini/del.ico></a></td>";
	echo '</tr>';
}
echo '</table>';
CloseTable();
