...
OpenTable();
			...
			if ($nr != 0){
				...
				echo '<tr>';
				echo "<td valign=middle align=center><font face=verdana size=3><a href='gestionale.php?name=lloyds&subname=offerte&act=explode&id=$_GET[id]&view=CGA&nomefile=$row[filename]'>Click<br>to<br>Print</a></td>";
				...
				$sql = "SELECT * FROM nuke_offerte_detail_cga WHERE field1 = '".$row[id]."' and idofferta='$_GET[id]'";
				$rs_cga = $db->sql_query($sql);
				$nr_cga = $db->sql_numrows($rs_cga);
				$cga = mysql_fetch_assoc($rs_cga);
				if ($nr_cga == 1) {$img='circle_green.png';$add='0';$remove='1';}
				if ($nr_cga == 0) {$img='circle_red.png';$add='1';$remove='0';}
				echo "<td align=right width=15%>
							<img src=immagini/$img>";
							if ($add=='1') {echo "<a href=$_SERVER[PHP_SELF]?name=lloyds&subname=offerte&act=explode&id=$_GET[id]&azioni=aggiungicga&view=CGA&cgaid=$row[id]#sonoqui1><img border=0 src=immagini/add.png></a>";}
							if ($add=='0') {echo "<img border=0 src=immagini/add_grey.png>";}
							if ($remove=='1') {echo "<a href=$_SERVER[PHP_SELF]?name=lloyds&subname=offerte&act=explode&id=$_GET[id]&azioni=eliminacga&view=CGA&cgaid=$cga[id]#sonoqui1><img border=0 src=immagini/minus.png></a>";}
