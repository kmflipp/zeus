...
OpenTable();
echo '<p>';
echo '<input type=button value="Nuovo Record" onclick="location.href=' . chr(39) . 'gestionale.php?name=detail&act=new&id=' . $id . chr(39) . '" style="font-family: Verdana; font-size: 10px">';
echo '<input type=button value="Mostra tutti i record" onclick="location.href=' . chr(39) . 'gestionale.php?name=detail&id=' . $id . chr(39) . '" style="font-family: Verdana; font-size: 10px">';
echo '</p>';
CloseTable();
...
if ($act == 'new') {
	echo '<form action=gestionale.php method=get><input type=hidden name=name value=detail><input type=hidden name=act value=savnew><input type=hidden name=id value='.$id.'>';
	echo '<tr>';
...
while ($row = $db->sql_fetchrow($rs))
{
	echo '<tr>';
		...
		echo "<td align=center valign=middle><font face=verdana size=2><a href=gestionale.php?name=detail&act=del&id=" . $row[id] . "&detail=" . $row[detail] . "&value=" . $row[value] . "&um=" . $row[um] . " " . $confirm . "><img border=0 src=immagini/del.ico></a></td>";
	echo '</tr>';
}
echo '</table>';
CloseTable();
