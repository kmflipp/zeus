...
echo "
<p>
<input type=button value='Nuovo Record' onclick=location.href='gestionale.php?name=lloyds&subname=offerte&act=new&idcliente=$id' style='font-family: Verdana; font-size: 10px'>
</p>";
echo '<table width=100% border=1 cellpadding=0 cellspacing=0>';
$sql = "SELECT * FROM nuke_offerte WHERE field3=$id or field4=$id or field5=$id";
$rs = $db->sql_query($sql);
echo '<tr>';
	echo '<th width=15%><font face=verdana size=2>Numero proposta</th>';
...
while ($row = $db->sql_fetchrow($rs))
{
	echo '<tr>';
		...
		echo "<td align=center valign=middle><font face=verdana size=2><a href=gestionale.php?name=lloyds&subname=offerte&act=explode&id=$row[id]><img border=0 src=immagini/select.png></a></td>";
	echo '</tr>';
}
echo '</table>';
CloseTable();
